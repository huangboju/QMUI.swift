...
class QMUIQQEmotionManager {
    ...
    init() {
        emotionView.didSelectEmotionBlock = { [weak self] index, emotion in
    ...
    func deleteEmotionDisplayNameAtCurrentSelectedRange(force: Bool) -> Bool {
        ...
        let selectedRange = self.selectedRangeForBoundTextInput
...
extension String {
    mutating func replace(_ range: Range<Int>, with str: String) -> String {
        ...
        let endIndex = self.index(self.startIndex, offsetBy: range.upperBound, limitedBy: self.endIndex) ?? self.startIndex
        self.replaceSubrange(startIndex..<endIndex, with: str)
        return self
    }
}
