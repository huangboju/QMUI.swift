...
class QMUIMoreOperationController: UIViewController {
    ...
    public func showFromBottom() {
        ...
        modalPresentationViewController.showingAnimation = { [weak self] (dimmingView, containerBounds, keyboardHeight, contentViewFrame, completion) in
        ...
        modalPresentationViewController.hidingAnimation = { [weak self] (dimmingView, containerBounds, keyboardHeight, completion) in
            UIView.animate(withDuration: 0.25, delay: 0, options: .curveOut, animations: {
    ...
    private func hideToBottomCancelled(_ cancelled: Bool) {
        ...
        modalPresentedViewController?.hide(with: true, completion: { [weak self] (finished) in
    ...
    @objc func handleMaskControlEvent(_ sender: UIControl) {
        hideToBottomCancelled(true)
    }
    
    @objc func handleCancelButtonEvent(_ sender: QMUIButton) {
        hideToBottomCancelled(true)
    }
