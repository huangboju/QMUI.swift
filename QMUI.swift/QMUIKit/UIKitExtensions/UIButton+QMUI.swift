...
public extension UIButton {
    ...
    public func qmui_setTitleAttributes(_ attributes: [NSAttributedStringKey: Any], for state: UIControlState) {
        ...
        if attributes.isEmpty {
            self.qmui_titleAttributes.removeValue(forKey: state.rawValue)
        ...
        self.qmui_titleAttributes[state.rawValue] = attributes
        ...
        if self.qmui_titleAttributes.count > 0 && self.qmui_titleAttributes[UIControlState.normal.rawValue] != nil {
            self.qmui_setTitleAttributes([:], for: .normal)
        }
        
    }

    private func qmui_setTitle(_ title: String, for state: UIControlState) {
        self.qmui_setTitle(title, for: state) // 方法替换之后相当于调用系统的setTitle方法
        
        if title.length <= 0 || self.qmui_titleAttributes.count == 0 {
            return
        }
        
        if state == .normal {
            for attribute in self.qmui_titleAttributes {
    ...
    private func qmui_setTitleColor(_ color: UIColor, for state: UIControlState) {
        self.qmui_setTitleColor(color, for: state)
        
        if let attribute = self.qmui_titleAttributes[state.rawValue] {
            var newAttribute = attribute
            newAttribute[NSAttributedStringKey.foregroundColor] = color
            self.qmui_setTitleAttributes(newAttribute, for: state)
        }
    }
