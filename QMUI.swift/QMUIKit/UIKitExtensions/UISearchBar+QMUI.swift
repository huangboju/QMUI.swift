    ...
    private func fixSafeAreaInsetsStyle() {
        ...
        if #available(iOS 11.0, *) {
            ...
            if var newframe = qmui_cancelButton()?.frame {
                let _ = newframe.setX(newframe.minX - safeAreaInsets.left)
            }
            ...
            let _ = qmui_textField()?.frame.setX(newLeft)
            ...
            if (isScopeBarShowingAtRightOfTextField) {
                ...
                let _ = qmui_segmentedControl()?.superview?.frame.setX(newLeft)
                let _ = qmui_segmentedControl()?.superview?.frame.setWidth(newWidth)
            ...
            if (!qmui_isActive() && showsScopeBar) {
                ...
                let _ = qmui_segmentedControl()?.frame.setX(newLeft)
                let _ = qmui_segmentedControl()?.frame.setWidth(newWidth)
        ...
        if #available(iOS 11.0, *) {
            ...
            if (isActive && isFrameError) {
                ...
                if let originImage = backgroundView.layer.contents {
                    ...
                    let _ = backgroundView.frame.setHeight(newHeight)
