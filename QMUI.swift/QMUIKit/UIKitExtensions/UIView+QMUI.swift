...
extension UIView {
    ...
    public var qmui_minXWhenCenterInSuperview: CGFloat {
        return self.superview?.bounds.width.center(with: self.frame.width) ?? 0
    }
...
extension UIView {
    ...
    public func qmui_hasOverrideUIKitMethod(_ selector: Selector) -> Bool {
        ...
        var viewSuperclasses = [UILabel.self,
                                UIButton.self,
        ...
        for aClass in viewSuperclasses {
            if self.qmui_hasOverrideMethod(selector: selector, of: aClass) {
                return true
            }
        }
