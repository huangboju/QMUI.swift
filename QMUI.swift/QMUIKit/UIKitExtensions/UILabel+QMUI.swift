...
extension UILabel: SelfAware {
    ...
    static func awake() {
        DispatchQueue.once(token: _onceToken) {
            ReplaceMethod(NSClassFromString("UILabel")!, #selector(setter: text), #selector(qmui_setText(_:)))
            ReplaceMethod(NSClassFromString("UILabel")!, #selector(setter: attributedText), #selector(qmui_setAttributedText(_:)))
        }
    }
